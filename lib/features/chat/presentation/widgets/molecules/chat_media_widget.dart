...
class ChatMediaWidget extends StatelessWidget {
  ...
  Widget _buildMediaGrid(BuildContext context) {
    ...
    if (mediaUrls.length == 1) {
      return AspectRatio(
        aspectRatio: 4 / 5,
        child: _buildMediaItem(mediaUrls[0], colorScheme, 0),
      );
    }
    ...
    if (mediaUrls.length == 2) {
      return SizedBox(
        ...
        child: Row(
          children: [
            Expanded(child: _buildMediaItem(mediaUrls[0], colorScheme, 0)),
            const SizedBox(width: 2),
            Expanded(child: _buildMediaItem(mediaUrls[1], colorScheme, 1)),
          ],
        ),
      );
    }
          ...
          children: [
            ...
            Expanded(
              child: Row(
                children: [
                  Expanded(
                    child: _buildMediaItem(mediaUrls[1], colorScheme, 1),
                  ),
                  const SizedBox(width: 2),
                  Expanded(
                    child: _buildMediaItem(mediaUrls[2], colorScheme, 2),
                  ),
                ],
              ),
            ),
      ...
      child: GridView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return Stack(
            fit: StackFit.expand,
            children: [
              _buildMediaItem(mediaUrls[index], colorScheme, index),
              if (isLastItem)
                Container(
  ...
  Widget _buildMediaItem(String url, ColorScheme colorScheme, int index) {
    return Hero(
      tag: 'chat_media_${identityHashCode(mediaUrls)}_$index',
      child: Image.network(
        url,
        fit: BoxFit.cover,
