...
void main() {
  ...
  testWidgets('dispatches AuthSignInRequested when form is submitted', (
    tester,
  ) async {
    ...
    final emailField = tester.widget<TextFormField>(textFieldByLabel('Email'));
  ...
  testWidgets(
    'dispatches AuthSignInWithGoogleRequested when Google button tapped',
    (tester) async {
      await pumpLoginScreen(tester);
      ...
      verify(
        () => mockBloc.add(const AuthSignInWithGoogleRequested()),
      ).called(1);
    },
  );
}
